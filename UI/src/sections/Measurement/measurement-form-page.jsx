...
// Place with other handlers
const openPreview = (src, alt = 'Preview') => {
  setPreviewSrc(src);
  setPreviewAlt(alt);
  setPreviewOpen(true);
};
...
const fetchUsers = async () => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('userFormDraft', JSON.stringify(serializable));
  }, [formValues]);
    ...
    canvas.toBlob((blob) => {
      if (blob) {
        if (blob.size <= 502400) {
          ...
          alert('Unable to compress photo below 500 KB.');
          closeCamera();
        }
      }
    }, 'image/jpeg', quality);
...
<TextField
  ...
  onChange={(e) => {
    const input = e.target.value.replace(/\D/g, '').slice(0, 10); // keep only digits, max 10
    setFormValues({ ...formValues, mobile: input });
...
/>
  ...
  <Box
    ...
    onClick={() => openPreview(photo.preview, `Preview ${index + 1}-${photoIdx + 1}`)}  // open dialog
  >
    <img
      src={photo.preview}
...
>
  <Box p={2}>
    ...
    >
      {/* Image scales within dialog keeping aspect ratio */}
      <img
        src={previewSrc}
        alt={previewAlt}
        style={{
